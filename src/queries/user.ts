...
const getMyCourses = async (
  ...
  return axios
    .get<
      HttpResponse<PaginatedResponse<ExamCourseProps>>
    >(endpoints().user.my_courses)
    .then((res) => res.data);
};
...
const startCourse = async (courseId: string, data: StartCourseDto) => {
  return axios
    .put<
      HttpResponse<ViewCourseProps>
    >(endpoints(courseId).user.start_course, data)
    .then((res) => res.data);
};
...
const getUpcomingEvents = async (params: {
  ...
  return axios
    .get<
      HttpResponse<PaginatedResponse<ExamCourseProps>>
    >(endpoints().user.upcoming_events)
    .then((res) => res.data);
};
...
const getMyPlans = async () => {
  return axios
    .get<HttpResponse<PaginatedResponse<MyPlan>>>(
      endpoints().user.get_my_plans,
      {
        params: {
          limit: 15,
          page: 1,
