...
const vetStudyPack = async (payload: VetStudyPackPayload) => {
  return axios
    .post<
      HttpResponse<VetStudyPackResp>
    >(endpoints().school.vet_study_pack, payload)
    .then((res) => res.data);
};
...
const getExamBundle = async (id: string) => {
  return axios
    .get<
      HttpResponse<SingleBundleResp>
    >(endpoints(id).school.get_single_exam_bundle)
    .then((res) => res.data);
};
export const useGetSingleExamBundleQuery = ({
  bundle_id,
}: {
  bundle_id: string;
}) => {
