...
export const BooleanChoiceAnswerType = ({
  options,
  selectAnswer,
  isAnswer,
}: Props) => {
  return (
    <div className="flex w-full flex-col gap-4">
      <p className="text-sm">Select the correct answer</p>
      <div className="grid w-full grid-cols-2 gap-4">
        {options?.map((answer, index) => (
          ...
          >
            ...
            <span
              className={`relative size-5 rounded-full border-2 before:absolute before:left-1/2 before:top-1/2 before:size-3 before:-translate-x-1/2 before:-translate-y-1/2 before:rounded-full before:bg-primary-400 ${isAnswer(answer.id) ? "border-primary-400 before:block" : "border-neutral-200 before:hidden"}`}
            ></span>
          </button>
        ))}
      </div>
