...
export const Navbar = () => {
	...
	return (
		<header
			...
			{/* Mobile navbar */}
			<nav className="container flex items-center justify-between px-3 lg:hidden">
				<Link href="/" className="relative h-[30px] w-[135px]">
					<Image src="/assets/images/classore.png" alt="classore" fill sizes="100%" />
				</Link>
				...
				<Sheet>
					...
					<SheetContent className="modal-primary flex w-[80%] flex-col gap-4 py-4">
						...
						<ul className="relative z-50 flex w-full flex-col gap-4 py-4">
							{links.map(({ name, href }) => (
								<li key={name}>
									<Link
										href={`#${href}`}
										className="link p-2 text-sm font-medium transition-colors duration-500 hover:text-primary-400">
										{name}
									</Link>
								</li>
							))}
						...
						<ul className="flex w-full flex-col gap-4 border-t border-t-neutral-200 pt-4">
							<Button
								asChild
								size="sm"
								variant="outline"
