      ...
      <DialogContent className="flex flex-col gap-4">
          ...
          <ul className="flex flex-col gap-1 pt-2">
            ...
            Number(lesson.quizes.at(-1)?.score) < chapter.bench_mark ? (
              <li className="text-xs">
                You need to score above ${chapter.bench_mark}% to qualify for
                next chapter
              </li>
            ) : null}
            ...
            {chapter.current_chapter_progress_percentage < 50 ? (
              <li className="text-xs">
                You must watch at least 50% of the video to unlock the current
                quiz
              </li>
            ) : null}
            ...
            {lesson.quiz_attempts_left <= 0 ? (
              <li className="text-xs">
                You have reached the maximum number of quiz attempts for this
                chapter. Please try again after {chapter.attempt_reset} hour
        ...
        <div className="flex w-full flex-col gap-2 rounded-lg bg-neutral-100 p-4 transition-all duration-700">
          ...
          <ul className="list-outside list-disc gap-2 pl-4 text-xs text-neutral-400">
            ...
            <li>
              Passing Score: Score {chapter.bench_mark}% or higher to unlock the
              next chapter.
            </li>
            <li>
              Time Limit: Complete the quiz within{" "}
              {chapter?.timer_hour ? `${chapter.timer_hour}hr` : ""}{" "}
              {chapter?.timer_minute}min.
            </li>
            <li>
              Quiz will be submitted automatically after the time limit expires.
            </li>
          </ul>
        </div>
        ...
        <div className="flex w-full items-center justify-end gap-4 border-t border-t-neutral-200 pt-4">
          ...
          <Button
            ...
            disabled={
              lesson.quiz_attempts_left <= 0 ||
              chapter.current_module_progress_percentage < 50
            }
            onClick={() =>
              router.push({
