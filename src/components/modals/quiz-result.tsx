...
export const QuizResultModal = ({
  ...
  const attempts_percentage =
    ((result.attempts_limit - result.attempts_left) /
      (result.attempts_limit ?? 3)) *
    100;
        ...
        <div className="flex w-full items-center gap-2 rounded-lg bg-neutral-100 px-4 py-3 text-neutral-400">
          ...
          <p className="max-w-[85%] text-xs">
            ...
            {result.is_passed ? (
              <>
                Congratulations! You{" "}
                <span className="font-bold text-[#319F43]">Passed</span> the
                Quiz.
              </>
            ) : (
              <>
                You <span className="font-bold text-[#E33629]">failed</span> the
