...
export const AddMoreCourseModal = ({
  ...
  const selectedSubjectsArray = selectedSubjects
    ? splitIntoArray(selectedSubjects)
    : [];
  ...
  const bundleSubjects = React.useMemo(() => {
    return subjects
      ...
      ?.filter(
        (subject) => !chosenSubjects?.find((s) => s.id === subject.subject_id),
      )
      .map((sub) => ({
