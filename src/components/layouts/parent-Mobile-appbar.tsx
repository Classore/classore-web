...
export function MobileParentDashboardLayout({ children, className, title }: MobileParentDashboardLayoutProps) {
  ...
  return (
    <>
      ...
      <div className="flex flex-col lg:hidden">
              ...
              <div className="w-full space-y-4 border-t border-neutral-200 pt-4">
                ...
                <div className="w-full space-y-2">
                  {parentHome?.my_wards.map((ward) => (
                    <WardItem key={ward.id} ward={ward} />
                  ))}
                </div>
              </div>
        ...
        <div className={cn("flex-1 overflow-y-auto bg-[#F6F8FA] p-4", className)}>
          {children}
        </div>
      </div>
    </>
