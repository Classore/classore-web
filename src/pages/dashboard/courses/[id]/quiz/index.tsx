...
const Page = () => {
  ...
  const lesson = chapter?.modules.find(
    (module) => module.id === String(module_id),
  );
  ...
  const {
    ...
    seconds:
      (chapter?.timer_hour ?? 0) * 3600 + (chapter?.timer_minute ?? 0) * 60,
            ...
            {error?.status === 403 ? (
              <>
                ...
                <p className="text-center text-sm text-neutral-400">
                  Your bundle plan has expired. Please renew your subscription
                  to continue accessing this bundle.
                </p>
              </>
            ) : (
              <>
          ...
          <div className="h-full w-full py-8">
            <div className="container mx-auto grid h-full w-full grid-cols-4 gap-5">
              ...
              <div className="col-span-2 flex h-fit w-full flex-col gap-4 rounded-xl border border-neutral-200 bg-gradient-to-b from-primary-100 from-0% via-white via-15% to-white to-100% p-5">
                ...
                {currentQuestion?.question_question_type.trim() ===
                "MULTICHOICE" ? (
