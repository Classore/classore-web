...
const Page = () => {
	...
	const maxSelectable = React.useMemo(() => {
		if (!examination_bundles) return 0;
		const bundle = examination_bundles?.data.find(
			(bundle) => bundle.examinationbundle_id.toLowerCase() === values.examination_bundle.toLowerCase()
		);
		if (!bundle) return 0;
